...
def init_app(app):
    # funcao de middleware para verificar o usuario
    @app.before_request
    def check_auth():
        # rotas que nao precisam de auth para serem acessadas
    ...
    def login():
        if request.method== 'POST':
            ...
            if user and check_password_hash(user.password, password): #ve se o hash do bd bate com a senha digitada
                ...
                flash(f'Falha no login, usuario ou senha incorreta!', 'danger')
                return redirect(url_for('login'))
    ...
    def galeria():
        ...
        if request.method == 'POST':
            ...
            # enviando o nome do arquivo para a funcao de verificacao de tipo
            if not arquivos_permitidos(file.filename):
                flash("tipo de arquivo nao permitido", "danger")
